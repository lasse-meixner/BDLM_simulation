...
```{r}
fit_random <- fit_model_dml_b2(data_random)
```

### Extracting results

```{r}
extract_results_dml_b2(fit_random, data_random$gamma, list(setting = "random", N = 200, P = 100, sigma = 1))
```
...
results_list <- lapply(names(fit_blrs), function(method_name) {
  extract_results_blr(fit_blrs[[method_name]], gamma = data_random$gamma, method_name = method_name, additional_results_info = list(setting = "random", N = 200, P = 100, sigma = 1))
})
...
```{r}
results <- run_simulation(model_type = c("BDML", "BLRs"), N = 200, P = 100, setting = "random", sigma = 1, simulation_size = 3)
```
...
```{r}
results_parallel <- run_simulation_parallel(model_type = c("BDML", "BLRs"), N = 200, P = 100, setting = "random", sigma = 1, simulation_size = 3, batch_size = 2, n_cores = 2)
